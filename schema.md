...
- **メディア一式**: すべて **`assets/`** に格納する（Markdown、画像、動画ファイル）。
- **type（Writer が作成するもの）**:
  - **write**: 記事（Markdown）。`assets.md` 必須。カード用画像は `assets.image`（`assets/{id}-card.*`）。
  - **picture**: 画像1枚。`assets.image` のみ（`assets/{id}.{ext}`）。
  - **movie**: 動画1本。`url` に外部URLまたは `/assets/...` のローカルファイル。
- **その他の type**（dev, music 等）: 手動で items.json に追加可能。表示・クリックは各ビューが対応する範囲で解釈。
- **サムネイル／メイン画像**: `assets.image` に `/assets/` 配下のパスを指定。後方互換のため `thumbnail` も参照されるが、新規は `assets.image` のみ使用する。

## Writer でのビュー別項目

スキーマレス設計のため、Writer では**ビューごとに必要な項目**をまとめて表示する。今後ビューが増えても、各ビューのスキーマに合わせて項目を追加・表示できる。

| ビュー | 使用する項目（Writer での入力） |
|--------|--------------------------------|
...
- **共通**: 全ビューで使う `id`（必須）。
- **Grid 用**: タイトルと画像のみ。画像はコンテンツ種別（記事・画像1枚・動画）で自動設定。
...
1. **assets.image**: 最優先。`/assets/` 配下の画像パス（例: `/assets/{id}.jpg`、`/assets/{id}-card.jpg`）。
