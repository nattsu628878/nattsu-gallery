 ...
 * 実行: cd apps/writer && node server.js → http://127.0.0.1:3333/ (ギャラリー), http://127.0.0.1:3333/writer/ (Writer)
...
// apps/writer の場合、リポジトリルートは writer の 2 階層上
  ...
  try {
    ...
    if (type === 'write') {
      ...
      if (topImage && topImage.filename && topImage.data) {
        ...
        item.assets = { md: `/pages/opus/assets/${trimmedId}.md`, image: `/pages/opus/assets/${fileName}` };
      } else {
        item.assets = { md: `/pages/opus/assets/${trimmedId}.md` };
      ...
      item.url = `/pages/opus/assets/${trimmedId}.md`;
