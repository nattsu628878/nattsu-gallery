...
function isOpusAssetReferenced(items, assetPath) {
  return items.some((item) => item.assets?.image === assetPath || item.url === assetPath);
}
async function unlinkOpusAssetIfUnused(assetPath, items) {
  if (!assetPath || !assetPath.startsWith("/opus/")) return;
  if (isOpusAssetReferenced(items, assetPath)) return;
  await promises.unlink(path.join(OPUS_ASSETS_DIR, path.basename(assetPath))).catch(() => {
  });
}
const GET = async () => {
  try {
...
const DELETE = async ({ url }) => {
  try {
    ...
    if (videoPath && videoPath.startsWith("/opus/")) {
      await promises.unlink(path.join(OPUS_ASSETS_DIR, path.basename(videoPath))).catch(() => {
      });
    }
    return new Response(JSON.stringify({ ok: true }), { status: 200 });
  } catch (error) {
